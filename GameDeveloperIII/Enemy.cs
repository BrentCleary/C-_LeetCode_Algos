public class Enemy
{
  ...
  public virtual void PerformAttack(Enemy Target, Attack ChosenAttack)
  {
    ...
    if(Target._Health > 0 && this._Health > 0 && Target._Health > 0)
    {
  ...
  {
    // Checks health of self
    if(_Health <= 0)
    {
      System.Console.WriteLine($"{this.Name} is unconscious.");
      return;
    }

    // checks if target health is below zero and returns (ends function)
    if(Target._Health <= 0)
    {
